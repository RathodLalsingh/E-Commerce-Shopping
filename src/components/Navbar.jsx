...
const Navbar = ({setData,cart}) => {
  ...
  return (
    ...
    <header className='sticky-top'>
        <div className="nav-bar">
            <Link to={'/'} className="brand">E-Commerce Shopping</Link>
  ...
  <span className="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
    {cart.length}
    <span className="visually-hidden">unreadable messages</span>
  </span>
</button>
            </Link>
        </div>

        {
          location.pathname == '/' && (
            ...
            className="items">{">="}90000</div>
            <div
            onClick={()=>filterByPrice(90000)}
            className="items">{">="}100000</div>
            <div
            onClick={()=>filterByPrice(90000)}
            className="items">{">="}150000</div>
            <div
            onClick={()=>filterByPrice(90000)}
            className="items">{">="}152000</div>
            <div
            onClick={()=>filterByPrice(90000)}
            className="items">{">="}160000</div>
